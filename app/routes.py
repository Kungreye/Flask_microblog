...
def explore():
    ...
    # reuse template 'index.html'; no form.


@app.route('/login', methods=['GET', 'POST'])
def login():
    if current_user.is_authenticated:       # is_authenticated, attr, not method.
    ...
    if form.validate_on_submit():
        ...
        next_page = request.args.get('next')              # for instance: /login?next=/index
        if not next_page or url_parse(next_page).netloc != '':
            return redirect(url_for('index'))
        return redirect(next_page)               # without url_for().
...
def register():
    if current_user.is_authenticated:       # property, not method.
        return redirect(url_for('index'))
...
def reset_password(token):
    if current_user.is_authenticated:
        return redirect(url_for('index'))
    user = User.verify_reset_password_token(token)      # staticmethod can be called by class.
    if not user:
        return redirect(url_for('index'))
...
def follow(username):
    ...
    current_user.follow(user)   # follow user, not username.
    db.session.commit()
...
def unfollow(username):
    ...
    if user == current_user:
        ...
        return redirect(url_for('user', username=username))
    current_user.unfollow(user)     # unfollow user, not username
    db.session.commit()
...
def translate_text():
    ...
    #print("request.form['source_language']=", request.form['source_language'])
    #print("request.form['dest_language']=", request.form['dest_language'])
